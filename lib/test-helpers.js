...
export const mockNote = (content, name, uuid) => {
  ...
  note.replaceContent = async (newContent, sectionObject) => {
    const sectionRegex = new RegExp(`(#+\\s*${ sectionObject.section.heading.text }\\s*\\n)[^#]*`, "gm");
    const heading = sectionRegex.match(note.body)[1];
    note.body = note.body.replace(sectionRegex, `${ heading }${ newContent }`);
  };
  return note;
}
