 ...
 */
export async function _syncBookHighlights(app, noteContents, bookNote, readwiseBookID, { readwiseBook = null, throwOnFail = false } = {}) {
  ...
  await _replaceContent(noteContents, "Summary", summaryContent);
  ...
  hlGroups = _distributeIntoSmallGroups(app, hlGroups, this.constants.maxBookHighlightsPerSection, _trimHighlights.bind(this));
  ...
  await _replaceContent(noteContents, "Sync History",`${ this.constants.updateStringPreface }${ _localeDateFromIsoDate(app, new Date()) }\n` + existingContent);
  return true;
}
