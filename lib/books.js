...
export async function _syncBookHighlights(app, noteContents, forceReprocess, readwiseFetchBooks, constants, dateFormat, bookNote, readwiseBookID, { readwiseBook = null, throwOnFail = false } = {}) {
  ...
  hlGroups = _distributeIntoSmallGroups(hlGroups, constants.bookConstants.maxBookHighlightsPerSection,
      constants.bookConstants.maxReplaceContentLength, _trimHighlights, dateFormat);
  ...
  await _replaceContent(noteContents, bookNote, "Sync History",`${ constants.bookConstants.updateStringPreface }${ _localeDateFromIsoDate(new Date(), dateFormat) }\n` + existingContent);
  return true;
}
