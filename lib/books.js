import { _getLastUpdatedTimeFromNote } from './dates.js';
import { _noteContent,
  _insertContent,
  _replaceContent,
  _sectionContent,
 ...
 */
export async function _syncBookHighlights(app, noteContents, forceReprocess, readwiseFetchBooks, constants, dateFormat, bookNote, readwiseBookID, { readwiseBook = null, throwOnFail = false } = {}) {
  ...
  const summaryContent = _bookNotePrefaceContentFromReadwiseBook(app, constants, dateFormat, readwiseBook, bookNote.uuid);
  ...
  const sortOrder = app.settings[constants.settingSortOrderName] || constants.defaultHighlightSort;
  ...
  hlGroups = _distributeIntoSmallGroups(hlGroups, constants.maxBookHighlightsPerSection,
      constants.maxReplaceContentLength, _trimHighlights, dateFormat);
  ...
  await _replaceContent(noteContents, bookNote, "Sync History",`${ constants.updateStringPreface }${ _localeDateFromIsoDate(new Date(), dateFormat) }\n` + existingContent);
  return true;
}
