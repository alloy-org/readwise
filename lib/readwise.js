...
export async function _getReadwiseBookCount(app, constants) {
  const bookIndexResponse = await _readwiseMakeRequest(app, `${ constants.readwiseBookIndexURL }?page_size=1`);
 ...
 */
export async function* _readwiseFetchBooks(app, {bookIdFilter=null, categoryFilter=null, dateFilter=null} = {}) {
  ...
  for await (const item of _readwisePaginateExportRequest(app, url)) {
 ...
 */
export async function* _readwisePaginateExportRequest(app, url) {
  let nextPage = false;

  while (true) {
    if (nextPage) url.searchParams.set("pageCursor", nextPage);
    const data = await _readwiseMakeRequest(app, url);
    if (data) {
 ...
 */
export async function* _readwisePaginateRequest(app, baseUrl) {
  let currentPage = 1;
  ...
  while (hasNextPage) {
    ...
    const data = await _readwiseMakeRequest(app, baseUrl);
    if (data) {
      for (const item of data.results) {
        if (this._abortExecution) break;
        yield item;
 ...
 */
export async function _readwiseMakeRequest(app, url) {
 ...
 */
export async function _ensureRequestDelta(app) {
