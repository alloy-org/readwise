    ...
    describe("with partial highlights already synced", () => {
      beforeEach(() => {
        ...
        bookNote = app.createNote(
            ...
            ["library"], expectedBookNoteContent, "3"
        );
        plugin._app = app;
      });
      ...
      it("should sync the extra highlight (Sync this Book)", async () => {
        await expect(plugin._syncThisBook(app, "3")).resolves.not.toThrow();
        expect(app._noteRegistry["3"].body).toContain(expectedBookNoteContentExtraHL);
      })
    });
  });
