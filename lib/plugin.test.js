 ...
 */
// TODO: add test for two subsequent syncs
...
async function mockReadwiseFetch(app, url) {
  const stringUrl = String(url);
  if (stringUrl.includes(plugin.constants.readwiseConstants.readwiseBookIndexURL)) {
  ...
  } else if (stringUrl.includes(plugin.constants.readwiseConstants.readwiseHighlightsIndexURL)) {
...
- Last synced at: .{20,}$
- Oldest update synced in: .{20,}$
- Next sync for content updated after: .{20,}$
...
`);
  ...
  describe("with no entries", () => {
    it("should sync all available books", async () => {
      ...
      await _insertContent(plugin._noteContents, dashboardNote, "")
          .then(result => {
            console.log(result);
          })
          .catch(error => {
      ...
      plugin._initialize(mockApp, {
        _getReadwiseBookCount(app, constants) {
          return Promise.resolve(1);
        },
        _readwiseFetchBooks: mockGetBook,
      })
      await plugin._syncAll(app);
      await _flushLocalNotes(app, plugin._noteContents);
      expect(dashboardNote.body).toMatch(expectedDashboardRegex);
    })
  });
})
...
describe("plugin2", () => {
  ...
  describe("with existing entries", () => {
    ...
    // Add the note content to the cache otherwise it's invisible
...
describe("plugin3", () => {
  ...
  describe("With a sparse note", () => {
    ...
    it("should sync all books", async () => {
      ...
      const bookNote = await app.createNote("", _noteTitleFromBook(readwiseBook1), bookNoteUUID, ["libary/books"]);
