...
describe("plugin", () => {
  ...
  const bookObject = _bookObjectFromReadwiseBook(null, readwiseBook1, bookNoteUUID);
  ...
  describe("with existing entries", () => {
    ...
    const dashboardNote = mockNote(content, plugin.constants.dashboardConstants.defaultDashboardNoteTitle, dashboardNoteUUID);
...
describe("plugin", () => {
  ...
  describe("With a sparse note", () => {
    plugin._initialize();
    const unformatted = `# Library Details
      - Last synced at: ${_localeDateFromIsoDate(null, new Date())}
      - Oldest update synced in: August 5, 2022
      - Next sync for content updated after: August 5, 2022
      - Readwise books imported into table: 18
      - Book count reported by Readwise: 25
    ...
    const expectedContent = unformatted.split("\n").map(n => n.replace(/^\s*/, "")).join("\n");
    const dashboardNote = mockNote(expectedContent, plugin.constants.dashboardConstants.defaultDashboardNoteTitle, dashboardNoteUUID);
    _insertContent(plugin._noteContents, dashboardNote, expectedContent)
        .then(result => {
          console.log(result);
        })
        .catch(error => {
    ...
    _insertContent(plugin._noteContents, bookNote, "")
        .then(result => {
          console.log(result);
        })
        .catch(error => {
...
describe("plugin", () => {
  ...
  describe("A dense note", () => {
    ...
    const dashboardNote = mockNote(startNoteContent, plugin.constants.dashboardConstants.defaultDashboardNoteTitle, dashboardNoteUUID);
    const app = mockApp(dashboardNote);
