...
function mockGetBook(readwiseBooks) {
  async function* readwiseGetBook() {
...
`;
  ...
  describe("with no entries", () => {
    ...
    it("should sync nothing more", async() => {
      // ------------------------------------------------------------------------------------------
      // Test a subsequent empty sync as well
      await plugin._syncAll(app);
      expect(dashboardNote.body.split("\n").slice(0, 4).join("\n")).toMatch(expectedDashboardRegex);
      expect(dashboardNote.body).toContain(expectedDashboardContent);
      expect(app._noteRegistry["2"].body).toContain(expectedBookNoteContent);
    ...
    it("should sync an extra highlight", async () => {
      // ------------------------------------------------------------------------------------------
      // Test a subsequent sync with an extra highlight
      plugin.readwiseModule._readwiseFetchBooks = mockGetBook([readwiseBookTwoHighlights])
      await plugin._syncAll(app);
      expect(dashboardNote.body.split("\n").slice(0, 4).join("\n")).toMatch(expectedDashboardRegex);
      expect(dashboardNote.body).toContain(expectedDashboardContent);
    ...
    it("should sync an extra book", async () => {
      // ------------------------------------------------------------------------------------------
      // Test a subsequent sync with an extra book
...
`;
      ...
      plugin.readwiseModule._readwiseFetchBooks = mockGetBook([readwiseBookTwoHighlights, readwiseBook2OneHighlight]);
      plugin.readwiseModule._getReadwiseBookCount = () => (Promise.resolve(2));
      await plugin._syncAll(app);
      expect(dashboardNote.body.split("\n").slice(0, 4).join("\n")).toMatch(expectedDashboardRegex);
      expect(dashboardNote.body).toContain(expectedDashboardContentExtraBook);
      expect(app._noteRegistry["2"].body).toContain(expectedBookNoteContentExtraHL);
