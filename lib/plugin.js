...
const plugin = {
  ...
  async _syncAll(app, categoryFilter) {
    ...
    try {
      ...
      if (!dashboardNoteContents.includes(this.constants.dashboardLibraryDetailsHeading)) {
        await _insertContent(this._noteContents, "# " + this.constants.dashboardLibraryDetailsHeading + "\n");
      }
      ...
      if (readwiseBookCount) {
        await _updateDashboardDetails(app, dashboard, this.constants.dashDetails, details, { bookCount: readwiseBookCount });
      }
      ...
      for await (const readwiseBook of _readwiseFetchBooks(app, {dateFilter, categoryFilter})) {
        ...
        // Edits book note
        if (typeof noteContents[bookNote.uuid] === "undefined") {
          // Don't load too many notes in memory
          if (Object.keys(noteContents).length >= this.constants.maxBookLimitInMemory) {
      ...
      await _updateDashboardDetails(app, dashboard, this.constants.dashDetails, details, {tableRowCount});
  ...
  async _syncThisBook(app, noteUUID) {
    try {
      ...
      const success = await _syncBookHighlights(app, this._noteContents, bookId, { throwOnFail: true });
      if (this._useLocalNoteContents) {
