const plugin = {
  ...
  async _replaceContent(note, sectionHeadingText, newContent, { level = 1 } = {}) {
    ...
    if (this._useLocalNoteContents) {
      ...
      const sectionRegex = new RegExp(`^(#+\\s*${ sectionHeadingText }\\s*[\\r\\n]+)((?:(?!^#{1,${ throughLevel }}\\s)[\\s\\S])*)(?=[\\r\\n]+#+|$)`, "gm")
      const bodyContent = this._noteContents[note.uuid];
      const headingMatch = sectionRegex.exec(bodyContent);
      if (headingMatch) {
        this._noteContents[note.uuid] = bodyContent.replace(sectionRegex, `$1${ newContent }`);
      } else {
  ...
  async _flushLocalNotes(app) {
    for (const [uuid, content] of this._noteContents) {
  ...
  async _ensureBookInDashboardNoteTable(app, dashboardNote, bookRowContent) {
    ...
    let dashboardBookListMarkdown = this._sectionContent(await this._noteContent(dashboardNote), this.constants.dashboardBookListTitle);
    ...
    if (sectionInNote) {
      ...
      const existingSections = await this._sections(dashboardNote);
