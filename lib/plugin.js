const plugin = {
  constants: {
    ...
    maxBooksFetchedPerInvoke: 10, // Todo: Change back to 100 when publishing
  ...
  noteOption: {
    ...
    "Sync this book": async function (app, noteUUID) {
      try {
        ...
        this._syncBookHighlights(app, currentNote, bookId);
  ...
  _syncBookHighlights: async function (app, bookNote, readwiseBookID, readwiseBook = null) {
    ...
    if (!noteContent.includes("# Highlights") && readwiseBook) {
      bookNote.insertContent(this._bookNotePrefaceContentFromReadwiseBook(readwiseBook, bookNote));
    }
    ...
    for await (const highlight of this._readwiseGetAllHighlightsForBook(app, readwiseBookID, lastUpdatedAt)) {
      ...
      if (highlight.location || highlight.highlighted_at) {
        const details = []
        if (highlight.location) highlightContent += `**Location**: ${ highlight.location }\n`;
        if (highlight.highlighted_at) highlightContent += `**Highlighted at**: ${ this._americanDateFromIsoDate(highlight.highlighted_at) }\n`;
      }
      console.log("Inserting", highlightContent, "for", highlight, "in", readwiseBook?.title || "current note");
      bookNote.insertContent(highlightContent, { atEnd: true });
      highlightCount++;
    }
    ...
    await bookNote.insertContent(`- Updated at: ${ this._americanDateFromIsoDate(now) }`, { atEnd: true });
  },
  ...
  async _ensureBookNote(app, readwiseBook, dashboardNote) {
    ...
    const baseTag = app.settings[`Tag to assign to Readwise notes (default: '${ this.constants.baseTag }')`] || this.constants.baseTag;
    ...
    if (searchResults.length === 0) {
      ...
      const readwiseBookRow = this._noteContentsFromBook(readwiseBook, bookNote);
      await dashboardNote.insertContent(readwiseBookRow, { atEnd: true });
    } else {
  ...
  async* _readwisePaginateRequest(app, baseUrl) {
    ...
    while (hasNextPage) {
      ...
      console.log("Iterating over returned data", data);
