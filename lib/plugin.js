const plugin = {
  ...
  async _ensureBookInDashboardNoteTable(app, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    let readwiseTableContent = this._tableContentWithReadwiseBook(app, existingTable, dashboardNoteContents, readwiseBook, bookNoteUUID);

    readwiseTableContent = readwiseTableContent.replace(/[|]{2,100}/g, " |"); // Trade possible trailing pipes for a single pipe
    const doublePipeMatch = readwiseTableContent.match(/(\|\s*\|){2,10}/);
    if (doublePipeMatch) {
  ...
  /*******************************************************************************************/
  _tableContentWithReadwiseBook(app, tableContent, dashboardNoteContent, readwiseBook, bookNoteUUID) {
    ...
    const sectionContent = this._sectionContent(tableContent, sectionLabel) || "";
    let readwiseTableContent;
    const bookRowContent = this._bookRowContentFromReadwiseBook(app, readwiseBook, bookNoteUUID);
    const rowMatch = tableContent && this._updateStampRegex().exec(tableContent);
    if (rowMatch && rowMatch[1]) {
      const firstRowDateObject = this._dateObjectFromDateString(rowMatch[1]);
      if (firstRowDateObject && firstRowDateObject < this._dateObjectFromDateString(readwiseBook.updated)) {
        console.log("Inserting atop table");
        // Insert at the top of the table
        readwiseTableContent = tablePreambleContent + bookRowContent + tableContent + "\n\n";
      }
    }
    if (!readwiseTableContent) {
      readwiseTableContent = tablePreambleContent + tableContent + "\n" + bookRowContent + "\n";
    }
    return readwiseTableContent;
  },
