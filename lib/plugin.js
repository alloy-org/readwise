const plugin = {
  ...
  _sectionContent(noteContent, sectionHeadingText, { depth = null } = {}) {
    ...
    const sectionMatches = noteContent.matchAll(/^#+\s*[^#\n\r]+/mg);
    const indexes = Array.from(sectionMatches);
    const sectionMatch = indexes.find(m => m[0].includes(sectionHeadingText));
    if (!sectionMatch) throw new Error(`Could not find section with heading ${ sectionHeadingText }`);
    const level = sectionMatch[0].match(/^#/).length;
    ...
    const result = noteContent.slice(sectionMatch.index + sectionMatch[0].length + 1, endIndex);
    return result;
  },
  ...
  async _readwiseInsertDetail(app, tableContent, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    if (sectionContent) {
      return this._insertRowIntoExistingSection(sectionLabelText, sectionContent || "",
        bookRowContent, { readwiseBook });
      ...
      return this._insertRowIntoNewSection(sectionLabel, tableContent, bookRowContent, existingSections);
    }
  },
   ...
   * */
  _insertRowIntoNewSection(sectionLabel, sectionTableContent, bookRowContent, existingSections) {
    const sectionLabelText = sectionLabel.replace(/^[#\s]+/, "");

    console.log("Could not find any content for section", sectionLabelText);
    let eligibleSections = [];
    if (sectionLabel.match(/^#+/)[0]?.length === 2) {
  ...
  _insertRowIntoExistingSection(sectionLabel, noteContent, bookRowContent,
    ...
    if (precedingRow) {
      ...
      console.log("Inserting row in", sectionName, "after", precedingRow, "for", bookHighlightAt, "title", bookTitle, "in", content);
      return { content, sectionName };
    } else {
      const content = `${ this._tablePreambleContent() }${ bookRowContent }${ this._tableStrippedPreambleFromTable(tableBody).trim() }\n`;
      console.log("No row before ", sectionName, "highlight", bookHighlightAt, "title", bookTitle, "in", tableBody, ". Returning", content, "Nothing more.");
      return { content, sectionName };
    }
  },
  ...
  async _migrateBooksToSections(app, dashboardNote) {
    let dashboardBookListMarkdown = this._sectionContent(await dashboardNote.content(), this.constants.dashboardBookListTitle);
    let bookListRows = dashboardBookListMarkdown.matchAll(/^(\|\s+![^\n]+)\n/gm);

    if (dashboardBookListMarkdown.match(/^##/m)) {
      console.log("Book list is already in sections, not migrating");
      return;
    } else if (!dashboardBookListMarkdown.includes(this.constants.unsortedSectionTitle)) {
      await dashboardNote.replaceContent(`## ${ this.constants.unsortedSectionTitle }\n${ dashboardBookListMarkdown }`, this._sectionFromHeadingText(this.constants.dashboardBookListTitle));
      dashboardBookListMarkdown = this._sectionContent(await dashboardNote.content(), this.constants.dashboardBookListTitle);
    }

    let content, sectionName;
    for (const bookMatch of bookListRows) {
      const bookRowContent = bookMatch[0];
      const dateString = (new RegExp(`^(?:\\|[^|]+){${ this._columnsBeforeUpdateDate }}\\|([^|]+)`)).exec(bookRowContent)[1];
      const titleString = (new RegExp(`^(?:\\|[^|]+){${ this._columnsBeforeTitle }}\\|([^|]+)`)).exec(bookRowContent)[1];
      ...
      dashboardBookListMarkdown = dashboardBookListMarkdown.replace(bookRowContent, ""); // Remove the row from the book list content
      await dashboardNote.replaceContent(dashboardBookListMarkdown, this._sectionFromHeadingText(this.constants.dashboardBookListTitle));

      if (sectionInNote) {
        ({ content, sectionName } = this._insertRowIntoExistingSection(sectionLabel, dashboardBookListMarkdown, bookRowContent,
          { bookTitle: titleString, bookHighlightAt: dateString }));
      } else { // No existing content for a sectionLabelText section, need to figure out where in the list of tables to place this one
        const existingSections = await dashboardNote.sections();
        ({ content, sectionName } = this._insertRowIntoNewSection(sectionLabel, dashboardBookListMarkdown, bookRowContent, existingSections));
      }
      ...
      await dashboardNote.replaceContent(content, this._sectionFromHeadingText(sectionName, { level }));
      dashboardBookListMarkdown = this._sectionContent(await dashboardNote.content(), this.constants.dashboardBookListTitle);
    }

    // Remove the old book list section
    const contentWithoutUnsortedHeading = dashboardBookListMarkdown.replace(new RegExp(`^##\s${ this.constants.unsortedSectionTitle }[\\r\\n]+`, "m"), "");
    await dashboardNote.replaceContent(contentWithoutUnsortedHeading, this._sectionFromHeadingText(this.constants.dashboardBookListTitle));
