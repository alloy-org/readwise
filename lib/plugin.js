const plugin = {
  ...
  async _ensureBookInDashboardNoteTable(app, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    const existingRowRegex = new RegExp(`^[|\s*]+${ readwiseBook.id }[^\n]+\n`, "m");
    if (dashboardNoteContents.match(existingRowRegex)) {
      const match = dashboardNoteContents.match(existingRowRegex);
      console.log("Existing dash match:", match);
    ...
    readwiseTableContent = readwiseTableContent.replace(/[|]{2,100}/g, "|"); // Remove any extra pipes
    console.debug("readwiseTableContent:", readwiseTableContent);
  ...
  async* _readwiseFetchBooks(app, dashboardNote) {
    ...
    if (dashboardContent) {
      console.debug("Found dashboardContent", dashboardContent);
      const updateThrough = dashboardContent.match(new RegExp(`${ this.constants.updateStringPreface }(.*)`));
      if (updateThrough) {
        console.log("Found updateThrough match", updateThrough);
  ...
  async _updateDashboardDetails(app, dashboardNote, dashboardNoteContent, updateIndex, { bookCount = null } = {}) {
    ...
    let lastUpdatedAt;
    const updateColumnRegex = new RegExp(`(?:\|[\w]*)${ updateIndex }\|\s*([^|]+)\s*\|`);
