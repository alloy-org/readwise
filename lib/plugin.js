  ...
  _syncBookHighlights: async function (app, bookNote, readwiseBookID, readwiseBook = null) {
    ...
    for await (const highlight of this._readwiseGetAllHighlightsForBook(app, readwiseBookID, lastUpdatedAt)) {
      ...
      if (highlight.location) {
        highlightContent += `**Location**: [View ${ highlight.location } on Kindle](kindle://book?action=open&asin=${ readwiseBook.asin }&location=${ highlight.location }) or on [Readwise](https://readwise.io/bookreview/${ highlight.id })\n`;
      }
    ...
    if (newHighlightsList.length) {
      ...
      replaceContent = highlightsContent;
    }
