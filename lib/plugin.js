const plugin = {
  ...
  async _ensureBookInDashboardNoteTable(app, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    const bookRowContent = `| ${ readwiseBook.cover_image_url ? `![Book cover](${ readwiseBook.cover_image_url })` : "[No cover image]" } ` +
    ...
    const doublePipeMatch = readwiseTableContent.match(/(\|[^\n]*\|)/);
    if (doublePipeMatch) {
  ...
  async _getLastUpdatedTimeFromNote(app, noteHandle) {
    ...
    if (dateLine) {
      ...
      if (isNaN(result.getTime())) {
        console.log("Could not ascertain date from", dateLine);
        return null;
      }
    } else {
  ...
  async* _readwiseFetchBooks(app, dashboardNote) {
    ...
    if (dashboardContent) {
      ...
      if (updateThrough) {
        ...
        console.info("No starting date for updated highlights found among", dashboardContent)
      }
    }
  ...
  async _updateDashboardDetails(app, dashboardNote, dashboardNoteContent, { bookCount = null } = {}) {
    ...
    const lastUpdatedAt = await this._maxBookUpdatedAtFromNoteContent(dashboardNoteContent);
   ...
   * Keep details about imported books updated
   */
  _maxBookUpdatedAtFromNoteContent(noteContent) {
    // Derive the latest "updated at" time from existing table rows
    let lastUpdatedAt, dateMatch;
    const updateColumnRegex = new RegExp(`^(?:\\|[^|]*){${ this._columnsBeforeUpdateDate }}\\|\\s*([^|]+)\\s*\\|.*\\n`, "gm");
    while (dateMatch = updateColumnRegex.exec(noteContent)) {
      const parseableString = dateMatch[1].toLowerCase().replace(/\s?[ap]m/, "").replace(" at ", " ");
      const parsedDate = Date.parse(parseableString);
      const dateObject = new Date(parsedDate);
      if (isNaN(dateObject.getTime())) {
        console.log("dateMatch", dateMatch, "at [1]", dateMatch[1], "becomes ", parseableString, "and finally dateObject", dateObject);
      } else if (!lastUpdatedAt || dateObject > lastUpdatedAt) {
        lastUpdatedAt = dateObject;
      }
    }
    console.log("Found lastUpdatedAt", lastUpdatedAt, "aka", this._localeDateFromIsoDate(lastUpdatedAt))
