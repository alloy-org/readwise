const plugin = {
  constants: {
    ...
    pluginSettingsNoteTitle: "Readwise plugin settings",
  ...
  _syncBookHighlights: async function (app, bookNote, readwiseBookID, readwiseBook = null) {
    ...
    const summaryContent = this._bookNotePrefaceContentFromReadwiseBook(app, readwiseBook);
    ...
    const replaceContent = await this._bookHighlightsContentFromReadwiseBook(app, readwiseBook, highlightsContent, lastUpdatedAt);
  ...
  async _ensureBookNote(app, readwiseBook, dashboardNote) {
    ...
    if (searchResults.length === 0) {
      ...
      await this._ensureBookInDashboardNoteTable(dashboardNote, readwiseBook);
      return bookNote;
    } else {
   ...
   */
  _bookNotePrefaceContentFromReadwiseBook(app, book) {
    let asinContent = "";
    if (book.asin) {
      if (book.source.toLowerCase().includes("kindle")) {
        asinContent = `ASIN: [${ book.asin }](kindle://book?action=open&asin=${ book.asin })`;
      } else {
   ...
   */
  async _ensureBookInDashboardNoteTable(dashboardNote, readwiseBook) {
    const dashboardNoteContents = await dashboardNote.content();
    // Already inserted this book into our dashboard ToC
    if (dashboardNoteContents.includes(readwiseBook.title)) return;
  ...
  /*******************************************************************************************
   * Ensure that all highlights from a Readwise book are in the
   */
  async _bookHighlightsContentFromReadwiseBook(app, readwiseBook, existingHighlightsContent, lastUpdatedAt) {
    ...
    for await (const highlight of this._readwiseGetAllHighlightsForBook(app, readwiseBookID, lastUpdatedAt)) {
      ...
      if (highlight.note) highlightContent += `\n- **Note**: ${ highlight.note }\n`;
  ...
  async _ensureRequestDelta(app) {
    ...
    if (this._lastRequestTime) { // Check if there was a previous request
      ...
      if (this._requestsCount >= this.constants.rateLimit) {
        ...
        app.alert(`Waiting for ${ Math.floor(waitTime / 1000) } seconds to satisfy Readwise API limit... Hang tight. ⏳`); // Alert the user about the waiting time
