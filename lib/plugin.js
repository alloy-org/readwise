const plugin = {
  ...
  _syncBookHighlights: async function (app, bookNote, readwiseBook) {
    ...
    if (!noteContent.includes("# Highlights")) {
      bookNote.insertContent(`![book title](${ readwiseBook.cover_image_url })\n# Book Details\n**Title**: ${ readwiseBook.title }\n` +
      `**Author**: ${ readwiseBook.author }\n**Category**: ${ readwiseBook.category }\n`);
      if (readwiseBook.url) bookNote.insertContent(`**Readwise URL**: ${ readwiseBook.url }\n\n\n# Highlights\n`);
    }
    ...
    for await (const highlight of this._readwiseGetAllHighlightsForBook(app, readwiseBook.id, lastUpdatedAt)) {
      ...
      console.log("Inserting", highlightContent, "for", highlight, "in", readwiseBook.title);
    ...
    await bookNote.insertContent(`- Updated at: ${ now }`, { atEnd: true });
  },
  ...
  async _ensureBookNote(app, readwiseBook, dashboardNote) {
    ...
    if (searchResults.length === 0) {
      ...
      const readwiseBookRow = `![Book cover](${ readwiseBook.cover_image_url })\n` +
        ...
        `- Author: ${ readwiseBook.author }\n` +
        `- Category: ${ readwiseBook.category }\n` +
        `- Source: [${ readwiseBook.source }](${ readwiseBook.source_url })\n` +
