const plugin = {
  ...
  async _ensureBookInDashboardNoteTable(app, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    if (!dashboardNoteContents.includes(this.constants.dashboardBookListTitle)) {
      ...
      existingTable = this._sectionContent(dashboardNoteContents, this.constants.dashboardBookListTitle, { depth: 1 }) || "";
    }
    ...
    existingTable = existingTable.replace(existingRowRegex, "");

    await this._sortedInsertReadwiseBook(app, existingTable, dashboardNote, readwiseBook, bookNoteUUID);
  ...
  async _sortedInsertReadwiseBook(app, tableContent, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    let sectionContent = this._sectionContent(tableContent, sectionTarget) || "";

    if (sectionContent) {
      await this._insertRowIntoExistingSectionContent(sectionLabelText, sectionContent, dashboardNote, readwiseBook, bookRowContent);
    } else { // No existing content for a sectionLabelText section, need to figure out where in the list of tables to place this one
      await this._insertRowIntoNewSection(sectionLabel, dashboardNote, dashboardNoteContent, readwiseBook, bookRowContent);
    }
  },

  /*******************************************************************************************/
  async _insertRowIntoNewSection(sectionLabel, dashboardNote, dashboardNoteContent, readwiseBook, bookRowContent) {
    const existingSections = await dashboardNote.sections();
    const sectionLabelText = sectionLabel.replace(/^[#\s]+/, "");
    let existingBookList = this._sectionContent(dashboardNoteContent, this.constants.dashboardBookListTitle) || "";

    console.log("Could not find any content for section", sectionLabelText, "sectionTarget", sectionTarget);
    // Insert a new section/table along with this book
    ...
    // Replace old preamble(s) with current one
    existingBookList = existingBookList.replace(/^\s*\|\s*Cover[^\n]+\n[|\-\s]+\n/gm, this._tablePreambleContent());
    let bookListContent;
    ...
    if (sectionBeforeLabel) { // There is a section before this section
      ...
      console.log("Inserting new table ahead of", existingBookList);
  ...
  async _replaceSectionContent(dashboardNote, sectionTextLabel, newContent) {
    let replaceSection = this._sectionFromHeadingText(sectionTextLabel);

    newContent = this._tableStrippedPreambleFromTable(newContent);
    ...
    newContent = `${ this._tablePreambleContent() }${ newContent }`

    console.log("Replacing section", sectionTextLabel, "with", newContent.length, "characters");
    console.debug("New content", newContent);
