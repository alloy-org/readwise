const plugin = {
  ...
  noteOption: {
    ...
    "Sync all": async function (app, noteUUID) {
      try {
        ...
        await this._ensureUpdatedLibraryDetails(app, dashboardNote);
        let bookCount = 0;
  ...
  async _ensureBookNote(app, readwiseBook, dashboardNote) {
    ...
    console.log(`_ensureBookNote(${ readwiseBook.title }, ${ dashboardNote }) with baseTag`, baseTag);
  ...
  async _ensureBookInDashboardNoteTable(app, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    if (!dashboardNoteContents.includes(this.constants.dashboardBookListTitle)) {
      ...
      [/\|\s?\*\*Cover\*\*\s?\|\s?\*\*Book Title\*\*\s?\|\s?\*\*Author\*\*\s?\|\s?\*\*Category\*\*\s?\|\s?\*\*Source\*\*\s?\|\s?\*\*Highlights\*\*\s?\|\s?\*\*Updated\*\*\s?\|\s?\*\*Other Details\*\*\s?\|\n/gm,
        /\|-\|-\|-\|-\|-\|-\|-\|-\|\n/gm,
    ...
    const bookRowContent = `| ![${ readwiseBook.title } cover](${ readwiseBook.cover_image_url }) ` +
      ...
      `| ${ sourceContent } ` +
      `| ${ readwiseBook.num_highlights } highlight${ readwiseBook.num_highlights === 1 ? "" : "s" } ` +
  ...
  /*******************************************************************************************
   * Print updated library details that are placed atop main dashboard note
   */
  async _ensureUpdatedLibraryDetails(app, dashboardNote) {
