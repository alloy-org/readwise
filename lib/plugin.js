...
const plugin = {
  ...
  // --------------------------------------------------------------------------------------
  async _writeHighlight(app, highlight) {
  ...
  async _bookHighlightsFromNotes(app) {
    ...
    return bookNotes.map(bookNoteContent => {
      ...
      return {
        user_book_id: c.match(/^\(match-rbi\):[\s]?([\d])+/)[1],
        title: c.match(/^\(match-title\):[\s]?([^\n]+)/)[1],
        author: c.match(/^\(match-author\):[\s]?([^\n]+)/)[1],
  ...
  _highlightsFromNoteContent: noteContent => {
    const highlightContent = noteContent.match(/^###[\s]?([^\n]+)/)[1];
