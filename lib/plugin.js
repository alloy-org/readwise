const plugin = {
  ...
  async _readwiseInsertDetail(app, tableContent, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    if (sectionContent) {
      ...
      return this._insertRowIntoNewSection(sectionLabel, dashboardNote, tableContent, bookRowContent, { readwiseBook });
    }
  },
   ...
   * */
  async _insertRowIntoNewSection(sectionLabel, dashboardNote, sectionTableContent, bookRowContent) {
   ...
   * */
  async _insertRowIntoExistingSectionContent(sectionLabel, dashboardNote, sectionContent, bookRowContent,
  ...
  async _migrateBooksToSections(app, dashboardNote) {
    ...
    Array.from(bookListRows).forEach((rowMatch) => {
      ...
      if (sectionInNote) {
        ...
        ({ content, section } = this._insertRowIntoNewSection(sectionLabel, dashboardNote, bookListContent, bookRowContent));
      }
    });
