const plugin = {
  ...
  async _readwiseMakeRequest(app, url) {
    ...
    try {
      ...
      if (result) {
        ...
        await new Promise(r => setTimeout(r,this.constants.sleepSecondsAfterRequestFail * 1000));
  ...
  async _migrateBooksToSections(app, dashboardNote) {
    ...
    if (subSections.length && !subSections.find(heading => heading === this.constants.unsortedSectionTitle)) {
      ...
      await app.alert("Your Readwise library will be updated to split highlights into sections for faster future updates. This might take a few minutes if you have a large library.");
    }
    ...
    const unsortedContent = this._sectionContent(await dashboardNote.content(), this.constants.unsortedSectionTitle);
    if ((unsortedContent?.trim()?.length || 0) === 0) {
