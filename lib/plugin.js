...
const plugin = {
  ...
  _constants: {
    ...
    highlightBaseTag: app => (app.settings["Base tag (default is 'readwise')"] ? this._textToTagName(app.settings["Base tag (default is 'readwise')"]) || "readwise"),
  ...
  async _bookHighlightsFromNotes(app) {
    ...
    return bookNotes.map(bookNoteContent => {
      ...
      return {
        ...
        note_name: bookNoteContent.noteHandle.name,
        note_uuid: bookNoteContent.noteHandle.uuid,
