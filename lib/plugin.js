const plugin = {
  ...
  async _ensureBookNote(app, readwiseBook, dashboardNote) {
    ...
    console.log(`_ensureBookNote(${ app }, ${ readwiseBook.title }, ${ dashboardNote }) with baseTag`, baseTag);
  ...
  async _ensureBookInDashboardNoteTable(app, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    if (dashboardNoteContents.includes(readwiseBook.title)) {
      console.log("Readwise book", readwiseBook.title, "already looks to be present in dashboard note contents", dashboardNoteContents);
      return;
    }
