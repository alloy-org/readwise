const plugin = {
  constants: {
    defaultBaseTag: "capture/readwise",
  ...
  noteOption: {
    ...
    "Sync this book": async function (app, noteUUID) {
      try {
        ...
        console.log("What is requestPage?", requestPage);
  ...
  _syncBookHighlights: async function (app, bookNote, readwiseBookID, readwiseBook = null) {
    ...
    const summaryContent = this._bookNotePrefaceContentFromReadwiseBook(readwiseBook, bookNote);
    ...
    for await (const highlight of this._readwiseGetAllHighlightsForBook(app, readwiseBookID, lastUpdatedAt)) {
      ...
      if (highlight.location || highlight.highlighted_at) {
        ...
        if (highlight.highlighted_at) highlightContent += `**Highlighted at**: ${ this._americanDateFromIsoDate(highlight.highlighted_at) }\n`;
    ...
    bookNote.replaceContent(highlightsList.join("\n") + "\n\n", this._sectionFromHeadingText("Highlights"));
    ...
    await bookNote.replaceContent(`- Highlights updated at: ${ this._americanDateFromIsoDate(new Date()) }\n` + existingContent,
  ...
  async _ensureBookNote(app, readwiseBook, dashboardNote) {
    ...
    const baseTag = app.settings[`Tag to assign to Readwise notes (default: '${ this.constants.defaultBaseTag }')`] || this.constants.defaultBaseTag;
    ...
    if (searchResults.length === 0) {
      ...
      const tablePreambleContent = `| **Cover** | **Book Title** | **Author** | **Category** | **Source** | **Highlight Date** | **Other Details** |\n` +
        `|-|-|-|-|-|-|-|\n`;
      let existingTable = "";
      if (!dashboardNoteContents.includes("Book List")) {
        // Add a header to the dashboard note if it doesn't exist
        dashboardNote.insertContent("# Readwise Book List\n")
      } else {
        existingTable = this._sectionContent(dashboardNoteContents, "Readwise Book List");
        const removeRows = tablePreambleContent.split("\n").concat([ "| | | | | | | |\n" ]);
        console.log("removeRows", removeRows);
        removeRows.forEach(row => existingTable.replace(row, ""));
        console.log("Stripped table content: ", existingTable);
      }

      const readwiseTableContent = tablePreambleContent +
        `| ![${ readwiseBook.title } cover](${ readwiseBook.cover_image_url }) ` +
        `| [${ readwiseBook.title }](https://www.amplenote.com/notes/${ bookNote.uuid }) ` +
        `| ${ readwiseBook.author } ` +
        `| ${ readwiseBook.category } ` +
        `| ${ readwiseBook.source ? `[${ readwiseBook.source }](${ readwiseBook.source_url })` : "" } ` +
        `| ${ this._americanDateFromIsoDate(readwiseBook.last_highlight_at) } ` +
        `| [Readwise link](https://readwise.io/bookreview/${ readwiseBook.id }) |\n` +
        existingTable;

      await dashboardNote.replaceContent(readwiseTableContent, this._sectionFromHeadingText("Readwise Book List"));
    } else {
   ...
   */
  _bookNotePrefaceContentFromReadwiseBook(book, note) {
    return `![Book cover](${ book.cover_image_url })\n` +
      `- **[${ book.title }](https://www.amplenote.com/notes/${ note.uuid })**\n` +
      `- Book Author: ${ book.author }\n` +
      `- Category: ${ book.category }\n` +
      `- Source: [${ book.source }](${ book.source_url })\n` +
      `- ${ this._americanDateFromIsoDate(book.last_highlight_at) }\n` +
  ...
  /*******************************************************************************************/
   _americanDateFromIsoDate: isoDate => {
    try {
      const dateObj = new Date(isoDate);
      return `${ dateObj.toLocaleDateString("en-US") } ${ dateObj.toLocaleTimeString("en-US") }`;
    } catch (e) {
      console.error("There was an error parsing your date string", isoDate, e)
      return isoDate;
    }
  }
}
