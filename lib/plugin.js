const plugin = {
  ...
  _sectionContent(noteContent, headingTextOrSectionObject) {
    ...
    const indexes = Array.from(noteContent.matchAll(/^#+\s*([^#\n\r]+)/gm));
  ...
  async _migrateBooksToSections(app, dashboardNote) {
    ...
    const subSections = Array.from(dashboardBookListMarkdown.matchAll(/^##\s[\w]+/gm)).map(match =>
      match[0].trim()).filter(w => w);
