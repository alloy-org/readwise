...
const plugin = {
    ...
    "Sync all": async function (app, noteUUID) {
      try {
        ...
        for await (const book of this._readwiseFetchBooks(app)) {
          ...
          let newNote = await this._ensureBookNote(app, noteTitle, dashboardNote);
   ...
   */
  async _ensureBookNote(app, noteTitle, dashboardNote) {
    console.log(`_ensureBookNote(${ app }, ${ noteTitle }, ${ dashboardNote })`);
