const plugin = {
  ...
  _columnsBeforeUpdateDate: 6,
  _columnsBeforeTitle: 1,
  _dateFormat: null,
  ...
  async _ensureBookInDashboardNoteTable(app, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    if (!dashboardNoteContents.includes(this.constants.dashboardBookListTitle)) {
      ...
      // Remove any rows that ended up with extra columns, a bug that existed in June 6, 2023 version and before
      existingTable = existingTable.split("\n").map(row => row.replace(/(\|\s?[\w]+\s+){8}\|([|]+)/, "")).join("\n");
    }
    ...
    existingTable.replace(existingRowRegex, "");

    this._sortedInsertReadwiseBook(app, existingTable, dashboardNoteContents, readwiseBook, bookNoteUUID);
    await this._updateDashboardDetails(app, dashboardNote, await dashboardNote.content());
  },

  /*******************************************************************************************/
  async _sortedInsertReadwiseBook(app, tableContent, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    const tablePreambleContent = `| **Cover** | **Book Title** | **Author** | **Category** | **Source** | **Highlights** | **Updated** | **Other Details** | \n` +
      `|-|-|-|-|-|-|-|-|\n`;
    ...
    const sectionLabel = this._sectionLabelFromReadwiseBook(dashboardNoteContent, readwiseBook);
    let sectionContent = this._sectionContent(tableContent, sectionLabel) || "";
    const existingSections = await dashboardNote.sections();
    const existingBookList = this._sectionContent(dashboardNoteContent, this.constants.dashboardBookListTitle) || "";

    // This book will be placed in a heretofore unknown section
    if (!sectionContent) {
      const eligibleSections = existingSections.filter(section => (section.heading?.level === sectionLabel.match(/^#+/).length));
      let sectionBeforeLabel = null;
      for (let i = 0; i < eligibleSections.length; i++) {
        const compareSection = eligibleSections[i].heading?.text || "";
        if (compareSection.localeCompare(sectionLabel) < 0) { // Is this compareSection before sectionLabel?
      ...
      let bookListContent;
      if (!sectionBeforeLabel) {
        bookListContent = `${ sectionLabel }\n${ tablePreambleContent }\n${ bookRowContent }\n\n${ existingBookList }\n\n`;
      } else {
        const sectionBeforeContent = this._sectionContent(dashboardNoteContent, sectionBeforeLabel);
        bookListContent = existingBookList.replace(sectionBeforeContent,
          `${ sectionBeforeContent }\n\n${ tablePreambleContent }\n${ sectionBeforeLabel }\n${ bookRowContent }\n\n`);
      }
      await this._replaceSectionContent(dashboardNote, this.constants.dashboardBookListTitle, bookListContent);
    } else {
      ...
      if (rowBefore) {
        sectionContent = sectionContent.replace(rowBefore, `${ rowBefore }\n${ bookRowContent }`);
        console.log("Inserting row after", rowBefore, "for", readwiseBook.last_highlight_at, "title", readwiseBook.title, "in", sectionContent);
        await this._replaceSectionContent(dashboardNote, sectionLabelText, sectionContent);
      } else {
        const lastPreambleRow = tablePreambleContent.split("\n")[1];
        sectionContent = sectionContent.replace(lastPreambleRow, `${ lastPreambleRow }\n${ bookRowContent }`);
        console.log("No row before", readwiseBook.last_highlight_at, "title", readwiseBook.title, "in", sectionContent);
        await this._replaceSectionContent(dashboardNote, sectionLabelText, sectionContent);
      }
    }
  },
  ...
  /*******************************************************************************************/
  async _replaceSectionContent(dashboardNote, sectionTextLabel, newContent) {
    const replaceSection = this._sectionFromHeadingText(sectionTextLabel);
    newContent = newContent.replace(/[|]{2,100}/g, " |"); // Trade possible trailing pipes for a single pipe
    const doublePipeMatch = newContent.match(/(\|\s*\|){2,10}/);
    if (doublePipeMatch) {
      console.error("ERROR: _ensureBookInDashboardNoteTable() found double-pipes in table content:", doublePipeMatch);
    } else {
      console.debug("No double-pipes found")
    }

    console.log("Replacing section", sectionTextLabel, "with", newContent.length, "characters");
    await dashboardNote.replaceContent(newContent, replaceSection);
  },

  /*******************************************************************************************/
  _sectionLabelFromReadwiseBook(dashboardNoteContent, readwiseBook) {
    const bookCount = (new RegExp(`${ this.constants.booksImportedLabel }:\\s*(\\d+)`)).exec(dashboardNoteContent)?.at(1);
    console.log("Found", bookCount, "books imported");
    let sectionLabel;

    if (bookCount > this.constants.breakToYearBookCount) {
      if (readwiseBook.last_highlight_at) {
        const year = this._dateObjectFromDateString(readwiseBook.last_highlight_at).getFullYear();
        sectionLabel = `## ${ year } Highlights`
      } else {
        sectionLabel = "## No highlights yet";
      }
    } else if (bookCount > this.constants.breakToDecadeBookCount) {
      if (readwiseBook.last_highlight_at) {
        const year = (this._dateObjectFromDateString(readwiseBook.last_highlight_at).getFullYear() / 10).floor() * 10;
        sectionLabel = `## ${ year }s Highlights`
      } else {
        sectionLabel = "## No highlights yet";
      }
    } else {
      sectionLabel = this.constants.dashboardBookListTitle;
    }
  ...
  _bookRowContentFromReadwiseBook(app, readwiseBook, bookNoteUUID) {
    ...
    `| ${ readwiseBook.last_highlight_at ? ` + ${ this._localeDateFromIsoDate(app, readwiseBook.last_highlight_at) }` : "" } ` +
    `| ${ this._localeDateFromIsoDate(app, readwiseBook.updated) } ` +
