const plugin = {
  ...
  async _ensureBookInDashboardNoteTable(app, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    await dashboardNote.replaceContent(existingTable, replaceSection);
  ...
  async _sortedInsertReadwiseBook(app, tableContent, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    const sectionLabel = this._sectionLabelFromReadwiseBook(readwiseBook, bookCount || 0);
    ...
    if (sectionContent) {
      ...
      await this._insertRowIntoNewSection(sectionLabel, dashboardNote, tableContent, readwiseBook, bookRowContent);
    }
  },
  ...
  async _insertRowIntoExistingSectionContent(sectionLabelText, sectionContent, dashboardNote, readwiseBook, bookRowContent) {
    ...
    const rowRegex = /\|[^\n]+\n/g;
    let rowIter = 0;
    const rowMatches = rowRegex.exec(sectionContent);
    console.log("rowMatches", rowMatches);
    if (rowMatches) {
      for (let i = 0; i < rowMatches.length; i++) {
        rowIter += 1;
        const rowContent = rowMatches[i];
        console.log("rowMatch", rowMatch);
        if (rowIter > 50) {
          debugger;
          break;
        }
    ...
    if (rowBefore) {
      ...
      console.log("Before replace", await dashboardNote.content());
      await this._replaceSectionContent(dashboardNote, sectionLabelText, sectionContent);
      console.log("After replacing", sectionLabelText, "we have\n", await dashboardNote.content());
    }
  },
  ...
  /*******************************************************************************************
   * Return an appropriate section label for readwiseBook, given the count of books that currently reside within
   * dashboardNoteContent
   */
  _sectionLabelFromReadwiseBook(readwiseBook, bookCount) {
    let sectionLabel;
