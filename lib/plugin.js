const plugin = {
  ...
  async _readwiseInsertDetail(app, tableContent, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    if (sectionContent) {
      return this._insertRowIntoExistingSectionContent(sectionLabelText, sectionContent || "", dashboardNote,
      ...
      return this._insertRowIntoNewSection(sectionLabel, dashboardNote, tableContent, bookRowContent, existingSections);
    }
  },
   ...
   * */
  _insertRowIntoNewSection(sectionLabel, dashboardNote, sectionTableContent, bookRowContent, existingSections) {
    const sectionLabelText = sectionLabel.replace(/^[#\s]+/, "");
    ...
    return { section: this.constants.dashboardBookListTitle, content: bookListContent };
  },

  /******************************************************************************************
   * Return a { section: sectionReplaceToken, content: sectionContent } object
   * */
  _insertRowIntoExistingSectionContent(sectionLabel, dashboardNote, sectionContent, bookRowContent,
    ...
    if (rowMatches) {
      for (rowMatch in rowMatches) {
        ...
        if (sortBy === "highlight") {
          ...
          } else if (dateObject < this._dateObjectFromDateString(bookHighlightAt)) {
          ...
          if (rowTitle.localeCompare(bookTitle)) {
            rowBefore = rowContent;
          } else {
            break;
          }
    ...
    sectionLabel = sectionLabel.replace(/^[#\s]+/, "");
    if (rowBefore) {
      sectionContent = sectionContent.replace(rowBefore, `${ rowBefore }${ bookRowContent }`);
      console.log("Inserting row in", sectionLabel, "after", rowBefore, "for", bookHighlightAt, "title", bookTitle, "in", sectionContent);
      return { content: sectionContent, section: sectionLabel };
    } else {
      sectionContent = `${ this._tablePreambleContent() }${ bookRowContent }${ this._tableStrippedPreambleFromTable(sectionContent) }`;
      console.log("No row before ", sectionLabel, "highlight", bookHighlightAt, "title", bookTitle, "in", sectionContent);
      return { content: sectionContent, section: sectionLabel };
    }
  },
  ...
  async _migrateBooksToSections(app, dashboardNote) {
    const dashboardContent = await dashboardNote.content();
    let bookListContent = this._sectionContent(dashboardContent, this.constants.dashboardBookListTitle);
    let bookListRows = bookListContent.matchAll(/^(\|\s+![^\n]+)\n/gm);
    ...
    for (const bookMatch of bookListRows) {
      ...
      const sectionInNote = bookListContent.includes(`# ${ sectionLabel }`);
      bookListContent = bookListContent.replace(bookRowContent, ""); // Remove the row from the book list content
      if (sectionInNote) {
        ({ content, section } = this._insertRowIntoExistingSectionContent(sectionLabel, dashboardNote, bookListContent, bookRowContent,
          { bookTitle: titleString, bookHighlightAt: dateString }));
      } else { // No existing content for a sectionLabelText section, need to figure out where in the list of tables to place this one
        const existingSections = await dashboardNote.sections();
        ({ content, section } = this._insertRowIntoNewSection(sectionLabel, dashboardNote, bookListContent, bookRowContent, existingSections));
      }
      await dashboardNote.replaceContent(content, this._sectionFromHeadingText(section));
      bookListContent = this._sectionContent(dashboardContent, this.constants.dashboardBookListTitle);
    }
  },
