...
const plugin = {
  ...
  async _syncAll(app, categoryFilter) {
    ...
    try {
      ...
      for await (const readwiseBook of this.readwiseModule._readwiseFetchBooks(app, {dateFilter, categoryFilter})) {
        ...
        const success = await _syncBookHighlights(app, this._noteContents, this._forceReprocess, this.readwiseModule._readwiseFetchBooks, this.constants.bookConstants, dateFormat, bookNote, readwiseBook.id, { readwiseBook });
  ...
  async _syncThisBook(app, noteUUID) {
    try {
      ...
      const dateFormat = this._dateFormat || (app && app.settings[this.constants.settingDateFormat]) || "en-US";
      const success = await _syncBookHighlights(app, this._noteContents, this._forceReprocess, this.readwiseModule._readwiseFetchBooks, this.constants.bookConstants, dateFormat, currentNote, bookId, { throwOnFail: true });
