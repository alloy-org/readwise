...
const plugin = {
  ...
  _syncBookHighlights: async function (app, bookNote, readwiseBook) {
    ...
    for await (const highlight of this._readwiseGetAllHighlightsForBook(app, readwiseBook.id, lastUpdatedAt)) {
      ...
      if (noteContent.indexOf(highlight.text) === -1) {
        ...
        if (highlight.location || highlight.highlighted_at) {
          const details = []
          if (highlight.location) details.push(`Location: ${ highlight.location }`);
          if (highlight.highlighted_at) details.push(`Highlighted at: ${ highlight.highlighted_at }`);
          highlightContent += `\n - *Capture Details*  ${ details.join(" | ") }`;
        }
        await bookNote.replaceContent(highlightContent, { section: "Version History" });
        highlightCount++;
      }
    }
    ...
    await bookNote.insertContent(`- Updated at: ${ now }`);
  },
