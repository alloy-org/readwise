const plugin = {
  ...
  _localeDateFromIsoDate: (app, dateStringOrObject) => {
    try {
      ...
      const dateFormat = app.settings["Date format (default is 'en-US')"] || "en-US";
      let result = dateObject.toLocaleDateString(dateFormat);
      const recentDateCutoff = (new Date()).setDate((new Date()).getDate() - 3);
      console.log("Date result", result, "vs cutoff", dateObject > recentDateCutoff, "with time", dateObject.toLocaleTimeString(dateFormat));
      if (dateObject > recentDateCutoff) {
        result += " at " + dateObject.toLocaleTimeString(dateFormat);
      }
      return result;
    } catch (e) {
