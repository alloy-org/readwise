const plugin = {
  ...
  noteOption: {
    ...
    "Sync all": async function (app, noteUUID) {
      try {
        ...
        for await (const readwiseBook of this._readwiseFetchBooks(app, dashboardNote)) {
  ...
  async _ensureBookInDashboardNoteTable(app, dashboardNote, readwiseBook, bookNoteUUID) {
    ...
    const bookRowContent = `| ${ readwiseBook.cover_image_url ? `![${ readwiseBook.title } cover](${ readwiseBook.cover_image_url })` : "[No cover image]" } ` +
      `| [${ readwiseBook.title }](https://www.amplenote.com/notes/${ bookNoteUUID }) ` +
      `| ${ readwiseBook.author } ` +
      `| ${ readwiseBook.category } ` +
      `| ${ sourceContent } ` +
      `| [${ readwiseBook.num_highlights } highlight${ readwiseBook.num_highlights === 1 ? "" : "s" }](https://www.amplenote.com/notes/${ bookNoteUUID }#Highlights})${ readwiseBook.last_highlight_at ? ` last at ${ this._localeDateFromIsoDate(app, readwiseBook.last_highlight_at) }` : "" } ` +
    ...
    if (dashboardNoteContents.match(existingRowRegex)) {
      existingTable = existingTable.replace(existingRowRegex, (_match, offset) => {
        if (offset === 0) {
          return bookRowContent;
        } else {
          return "";
        }
      });
      readwiseTableContent = tablePreambleContent + existingTable + "\n";
    } else {
      readwiseTableContent = tablePreambleContent + bookRowContent + existingTable + "\n\n";
    }

    readwiseTableContent = readwiseTableContent.replace(/[|]{2,100}/g, " |"); // Trade possible trailing pipes for a single pipe
    const doublePipeMatch = readwiseTableContent.match(/(\|\s*\|)/);
    if (doublePipeMatch) {
    ...
    await this._updateDashboardDetails(app, dashboardNote, await dashboardNote.content(), updateIndex);
  },
   ...
   */
  async* _readwiseFetchBooks(app, dashboardNote) {
    console.log(`_readwiseFetchBooks(app, dashboardNote)`, dashboardNote);
    const url = new URL(`${ this.constants.readwiseBookIndexURL }/`);

    const dashboardContent = await dashboardNote.content();
    if (dashboardContent) {
      const updateThrough = dashboardContent.match(new RegExp(`${ this.constants.updateStringPreface }(.*)`));
      if (updateThrough) {
        const params = new URLSearchParams();
        params.append('updated__gt', updateThrough[1]);
        console.log("Found updateThrough match", updateThrough, "submitting as", updateThrough[1]);
        url.search = params;
      } else {
        console.info("No starting date for updated highlights found")
      }
    }
    ...
    if (this._lastRequestTime) { // Check if there was a previous request
      ...
      if (this._requestsCount >= this.constants.rateLimit) {
        ...
        const alertMessage = `Waiting for ${ Math.floor(waitTime / 1000) } seconds to satisfy Readwise API limit... 
          You can wait here, or click "OK" to dismiss, and we will update notes as you work. ⏳  
          
          Working concurrently while notes are being changed may lead to merge issues, 
          so we recommend minimizing note changes while a sync is in progress.`;
   ...
   */
  async _updateDashboardDetails(app, dashboardNote, dashboardNoteContent, updateIndex, { bookCount = null } = {}) {
    dashboardNoteContent = (dashboardNoteContent || "");

    // Derive the latest "updated at" time from existing table rows
    let lastUpdatedAt;
    const updateColumnRegex = new RegExp(`(?:\|[\w]*)${ updateIndex }\|\s*([^|]+)\s*\|`, "gm");
    console.log("Looking for dates in apparent dashboard note content:", dashboardNoteContent)
    const updateDates = dashboardNoteContent.match(updateColumnRegex);
    if (updateDates) {
      console.log("Found update matches", updateDates);
      updateDates.forEach(dateString => {
        dateString = dateString.toLowerCase().replace(/\s?[ap]m/, "").replace(" at ", " ");
        const parsedDate = Date.parse(dateString);
        ...
        if (isNaN(dateObject.getTime())) {
          console.log("Could not ascertain date from", dateString);
        } else if (!lastUpdatedAt || dateObject > lastUpdatedAt) {
          lastUpdatedAt = dateObject;
        }
      });
    } else {
      console.error("No update dates found in dashboard note content");
    }
    if (!lastUpdatedAt) {
      lastUpdatedAt = new Date();
      lastUpdatedAt.setFullYear(lastUpdatedAt.getFullYear() - 20);
    }
