const plugin = {
  ...
  _insertRowIntoExistingSection(sectionLabel, noteContent, bookRowContent,
    ...
    const sectionLabelText = sectionLabel.replace(/^[#\s]+/, "");
    const tableBodyRegex = new RegExp(`#+\\s*${ sectionLabelText }[\\r\\n]+.*[\\r\\n]+\\|[|\\-\\s]+[\\r\\n]+((?:\\|[^\\n]+[\\r\\n]+){1,})`, "gm");
    const tableMatch = tableBodyRegex.exec(noteContent);
    ...
    if (precedingRow) {
      ...
      const content = `${ this._tablePreambleContent() }${ bookRowContent }${ this._tableStrippedPreambleFromTable(tableBody) }`;
  ...
  async _migrateBooksToSections(app, dashboardNote) {
    const dashboardContent = await dashboardNote.content();
    let bookListSectionContent = this._sectionContent(dashboardContent, this.constants.dashboardBookListTitle);
    ...
    for (const bookMatch of bookListRows) {
      ...
      await dashboardNote.replaceContent(content, this._sectionFromHeadingText(sectionName, { level: sectionLabel.match(/^#*/)[0].length }));
