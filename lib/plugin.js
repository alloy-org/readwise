const plugin = {
  ...
  _sectionContent(noteContent, sectionHeadingText, { depth = null } = {}) {
    ...
    // Probably better replacement: `#+\\s*${ sectionHeadingText }[\\r\\n]((?:\\n|.)+)(?=\\n${ depth ? `#{1,${ depth }}\\s` : "#{1,10}\\s" }|$)`
    const regex = new RegExp(`#+\\s*${ sectionHeadingText }[\\r\\n]+([\\s\\S]+?)[\\r\\n]+${ depth ? `#{1,${ depth }}\\s` : "#{1,10}\\s" }`, "m");
  ...
  _insertRowIntoExistingSection(sectionLabel, noteContent, bookRowContent,
    ...
    const labelLineRegex = new RegExp(`^[#\\s]*${ sectionLabel }`, "m");
    ...
    const tableBody = noteContent.substring(sectionMatch.index).split("\n").filter(line => {
      if (contentEnded) {
        return false;
    ...
    if (precedingRow) {
      const content = tableBody.replace(precedingRow, `${ precedingRow.trim() }\n${ bookRowContent }`);
      ...
      let prefaceContent = "";
      if (Array.from(rowMatches).length === 0) prefaceContent = this._tablePreambleContent();
      const content = `${ prefaceContent }${ bookRowContent }${ this._tableStrippedPreambleFromTable(tableBody) }`;
      console.log("No row before ", sectionName, "highlight", bookHighlightAt, "title", bookTitle, "in", tableBody);
      return { content, sectionName };
    }
  },
  ...
  async _migrateBooksToSections(app, dashboardNote) {
    let bookListSectionContent = this._sectionContent(await dashboardNote.content(), this.constants.dashboardBookListTitle);
    let bookListRows = bookListSectionContent.matchAll(/^(\|\s+![^\n]+)\n/gm);

    if (bookListSectionContent.match(/^##/m)) {
      console.log("Book list is already in sections, not migrating");
      return;
    } else if (!bookListSectionContent.includes(this.constants.unsortedSectionTitle)) {
      await dashboardNote.replaceContent(`## ${ this.constants.unsortedSectionTitle }\n${ bookListSectionContent }`, this._sectionFromHeadingText(this.constants.dashboardBookListTitle));
      bookListSectionContent = this._sectionContent(await dashboardNote.content(), this.constants.dashboardBookListTitle);
    }
    ...
    for (const bookMatch of bookListRows) {
      ...
      const sectionInNote = bookListSectionContent.includes(sectionLabel);
      bookListSectionContent = bookListSectionContent.replace(bookRowContent, ""); // Remove the row from the book list content
      if (sectionInNote) {
        ({ content, sectionName } = this._insertRowIntoExistingSection(sectionLabel, bookListSectionContent, bookRowContent,
          { bookTitle: titleString, bookHighlightAt: dateString }));
      } else { // No existing content for a sectionLabelText section, need to figure out where in the list of tables to place this one
        const existingSections = await dashboardNote.sections();
        ({ content, sectionName } = this._insertRowIntoNewSection(sectionLabel, bookListSectionContent, bookRowContent, existingSections));
      }
    ...
    const contentWithoutUnsortedHeading = bookListSectionContent.replace(new RegExp(`^##\s${ this.constants.unsortedSectionTitle }[\\r\\n]+`, "m"), "");
