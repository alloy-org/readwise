const plugin = {
  ...
  _sectionContent(noteContent, headingTextOrSectionObject) {
    ...
    if (!sectionMatch) {
      console.error(`Checked for but could not find section with heading ${ sectionHeadingText }`);
      return null;
    }
  ...
  async _ensureBookInDashboardNoteTable(app, dashboardNote, bookRowContent) {
    ...
    dashboardBookListMarkdown = dashboardBookListMarkdown.replace(this._replaceRowRegexFromReadwiseBook(bookId), ""); // Remove the row from the book list content
    await this._replaceContent(dashboardNote, this.constants.dashboardBookListTitle, dashboardBookListMarkdown);
  ...
  _insertRowIntoNewSection(sectionLabel, sectionTableContent, bookRowContent, existingSections) {
    ...
    const labelIndentLevel = sectionLabel.match(/^#+/)[0]?.length;
    if (labelIndentLevel === 2) {
      eligibleSections = existingSections.filter(section =>
        (section.heading?.level === labelIndentLevel) && section.heading.text.trim().length && !section.heading.text.includes(this.constants.unsortedSectionTitle)
      ) || [];
    }
    console.log("Invoked _insertRowIntoNewSection to insert section", sectionLabelText, "Found", eligibleSections, "sections to check for one that might precede");
    ...
    if (sectionBeforeLabel) { // There is a section before this section
      ...
      console.log("Inserting new section", sectionLabel ," w/ content length", bookListContent.length, "after label", sectionBeforeLabel," amidst existing");
    } else { // No section before this one, put this first
      console.log("Inserting new table ahead of", sectionContentWithoutTable);
  ...
  /*******************************************************************************************/
  async _replaceContent(note, sectionHeadingText, content, { level = 1 } = {}) {
    const replaceTarget = this._sectionFromHeadingText(sectionHeadingText, { level });
    await note.replaceContent(content, replaceTarget);
  },
  ...
  async _migrateBooksToSections(app, dashboardNote) {
    let dashboardBookListMarkdown = this._sectionContent(await dashboardNote.content(), this.constants.dashboardBookListTitle);
    let bookListRows = Array.from(dashboardBookListMarkdown.matchAll(/^(\|\s*![^\n]+)\n/gm));
    if (bookListRows.length) {
      console.debug("Found", bookListRows.length, "books to migrate");
    } else {
      console.debug("No existing books found to migrate");
      return;
    }

    if (dashboardBookListMarkdown.match(/^##\s[\w]+/m)) {
